...
public class Meiga implements Comparable {
    // Atributos
    private String nome;
    ...
    public int compareTo(Object o) {
        Meiga m = (Meiga)o;
        return this.alcume.compareTo(m.alcume);
    }
    ...
    public String toString() {
        List<Feitizo> fList = new ArrayList<>(feitizos);
        fList.sort((f1,f2)->f2.getDificultade()-f1.getDificultade());

        return String.format("%s (%s)\nFeitizos: %s\nInventario: %s",
            nome,alcume,fList,inventario.keySet().toString());
    }
    ...
    public int lanzarFeitizos(){
        Map<String,Integer> tmpInv = new HashMap<>(inventario);
        ...
        for (Feitizo feitizo : fList) {
            System.out.println("-> Lanzando o feitizo: "+feitizo.getNome());

            for (String ingredient : feitizo.ingredientes) {
                if (!inventario.containsKey(ingredient)){
